...
A complete **Node.js + Express + MySQL** REST API backend for the Campus Lost & Found Portal web application.
...
```sql
users   → id, name, email, password (bcrypt), role, department, phone
items   → id, title, category, description, location, date_lost, image,
          color, brand, reward, type, status, user_id (FK)
claims  → id, item_id (FK), claimant_id (FK), message, contact,
          status, admin_note
```
